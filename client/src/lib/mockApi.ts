...
export const mockApi: RenuziApi = {
  ...
  async submitReconciliation(input: SubmitInput) {
    ...
    for (const row of rows) {
      ...
      if (manualNote) {
        row.Notes =
          row.Notes === '' ? `Manual: ${manualNote}` : `${row.Notes}; Manual: ${manualNote}`
      }
    }
  ...
  async updateMapping(code, patch) {
    ...
    const user = currentUser()
    requireRole(user, 'admin')
    const index = mappings.findIndex((entry) => entry.leverEdgeSkuCode === code.trim())
  ...
  async listAudit(query?: AuditQuery) {
    ...
    const filtered = auditLog
      .filter((entry) => {
        ...
        if (
          query?.location &&
          (entry.Location ?? '').toLowerCase() !== query.location.toLowerCase()
        )
          return false
