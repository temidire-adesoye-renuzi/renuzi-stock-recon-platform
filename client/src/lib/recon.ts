...
class Matcher {
  ...
  ): { method: Method; key: string | null; similarity: number | null; entry: SkuMappingEntry | null } {
...
function buildRow(group: Group, input: ReconcileInput): ReconciliationRow {
  ...
  return {
    ...
    SKU_Code:
      group.leverEdge?.sku ?? group.xero?.sku ?? group.physical?.sku ?? entry?.leverEdgeSkuCode ?? '',
    Item_Name:
  ...
  for (const row of rows) {
    ...
    if (row.Docked_Qty !== null) {
      ...
      if (row.Unit_Price_NGN !== null) {
        summary.dockedValueNGN = round3(summary.dockedValueNGN + row.Docked_Qty * row.Unit_Price_NGN)
      }
    }
