...
export default function AdminPage() {
  ...
  async function removeMapping(entry: SkuMappingEntry) {
    if (
      !window.confirm(`Delete the mapping for ${entry.leverEdgeSkuCode}? This cannot be undone.`)
    ) {
      return
    }
    setRowBusy(entry.leverEdgeSkuCode)
  ...
  async function createMapping() {
    ...
    if (
      !Number.isFinite(csFactor) ||
      csFactor <= 0 ||
      !Number.isFinite(dzFactor) ||
      dzFactor <= 0
    ) {
                      ...
                      <tbody>
                        ...
                        {filtered.length === 0 ? (
                          <tr>
                            <td
                              colSpan={9}
                              className="px-3 py-10 text-center text-xs text-neutral-500"
                            >
        ...
        >
          ...
          >
            ...
            {createError ? (
              <p
                role="alert"
                className="mx-5 mb-3 rounded-md border border-danger-25 bg-danger-10 px-3 py-2 text-xs text-[#C22F30]"
              >
                {createError}
              </p>
            ) : null}
