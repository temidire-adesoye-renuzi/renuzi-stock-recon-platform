...
export default function SubmitPage() {
  ...
  async function handleFile(
    ...
    try {
      ...
      toast(`${field === 'leveredge' ? 'LeverEdge' : field === 'xero' ? 'Xero' : 'Physical count'} file rejected — check the columns listed.`, 'error')
    } finally {
      setBusyField(null)
    }
  }
      ...
      <div className="flex-1 space-y-4 px-6 py-4">
        ...
        {statusLoading ? (
          ...
          <p className="rounded-lg border border-brand-25 bg-brand-10 px-4 py-2 text-xs text-brand">
            ...
            {new Date(submittedAt).toLocaleTimeString('en-GB', { timeZone: 'Africa/Lagos', hour: 'numeric', minute: '2-digit' })}{' '}
        ...
        <section aria-labelledby="uploads-heading">
          ...
          <div className={`mt-2 grid grid-cols-2 gap-4 ${locked ? 'pointer-events-none opacity-50' : ''}`}>
            <Dropzone
