...
function pctDelta(current: number, previous: number): { text: string; direction: 'up' | 'down' | 'flat' } {
...
export default function DashboardPage() {
  ...
  const kpis = useMemo(() => {
    ...
    return {
      ...
      atRiskDelta: {
        text:
          atRisk < prevAtRisk ? `${prevAtRisk - atRisk} cleared` : atRisk > prevAtRisk ? `+${atRisk - prevAtRisk} new` : 'steady',
                ...
                <ul className="space-y-2">
                  {audit.map((entry, index) => (
                    ...
                    >
                      ...
                      <p className="mt-0.5 truncate text-2xs text-neutral-500" title={entry.Details}>
                        {entry.Details}
                      </p>
                    </li>
                  ))}
