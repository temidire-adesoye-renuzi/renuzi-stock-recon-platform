...
export function SummaryBar({
  ...
  const submitDisabled = locked || submitting || missingNotes > 0 || !filesReady
    ...
    <div className="sticky bottom-0 z-20 border-t border-neutral-200 bg-white/95 px-6 py-3 backdrop-blur">
      <div className="flex items-center gap-8">
        ...
        <div className="ml-auto flex items-center gap-3">
          {locked ? (
            <p className="text-xs font-medium text-[#C22F30]">
              Locked for Executive Audit — resubmit tomorrow
            </p>
          ) : missingNotes > 0 ? (
          ...
          <Button variant="outline" size="md" onClick={onSaveDraft} disabled={locked || !filesReady}>
            Save draft
          </Button>
          <Button
            variant="accent"
            ...
            title={
              !filesReady
                ? 'Upload the LeverEdge and Xero exports first'
                : missingNotes > 0
                  ? 'Capture every mandatory variance note first'
                  : undefined
            }
          >
